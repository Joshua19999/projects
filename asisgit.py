...
class AsistenteGitGUI:
    ...
    def crear_tab_remoto(self):
        ...
        push_frame = ttk.LabelFrame(frame, text="Push (Subir Cambios)", padding="10")
        push_frame.pack(fill=tk.X, pady=10)
        ttk.Label(push_frame, text="Push automático a 'origin' con la rama actual", 
                 font=('Arial', 9, 'italic')).pack(anchor='w', pady=3)
        ttk.Button(push_frame, text="⬆️ Push Automático", command=self.push_auto, width=30).pack(pady=3)
        ttk.Button(push_frame, text="Push Personalizado...", command=self.push_personalizado, width=30).pack(pady=3)
        
        pull_frame = ttk.LabelFrame(frame, text="Pull (Descargar Cambios)", padding="10")
        pull_frame.pack(fill=tk.X, pady=10)
        ttk.Label(pull_frame, text="Pull automático desde 'origin' con la rama actual", 
                 font=('Arial', 9, 'italic')).pack(anchor='w', pady=3)
        ttk.Button(pull_frame, text="⬇️ Pull Automático", command=self.pull, width=30).pack(pady=3)
        
        fetch_label = ttk.Label(frame, text="Fetch (Obtener sin fusionar)", font=('Arial', 10, 'bold'))
        fetch_label.pack(pady=(15, 5))
        ttk.Button(frame, text="📡 Fetch", 
                  command=lambda: self.ejecutar_comando('git fetch origin'), width=35).pack(pady=5)
...
"""
    ...
    def push_auto(self):
        """Push automático a origin con la rama actual"""
        timestamp = datetime.now().strftime('%H:%M:%S')
        self.escribir_salida(f"\n[{timestamp}] ", 'timestamp')
        self.escribir_salida("⬆️ Iniciando push automático...\n", 'info')
        
        # Primero obtener la rama actual
        try:
            resultado = subprocess.run('git branch --show-current', shell=True, 
                                     capture_output=True, text=True, encoding='utf-8')
            rama_actual = resultado.stdout.strip()
            
            if rama_actual:
                self.escribir_salida(f"   Rama detectada: {rama_actual}\n", 'exito')
                self.escribir_salida(f"   Remoto: origin\n", 'info')
                self.ejecutar_comando(f'git push origin {rama_actual}')
            else:
                self.escribir_salida("❌ No se pudo detectar la rama actual\n", 'error')
                messagebox.showerror("Error", "No se pudo detectar la rama actual.\nAsegúrate de estar en un repositorio Git.")
        except Exception as e:
            self.escribir_salida(f"❌ Error: {e}\n", 'error')
            messagebox.showerror("Error", f"Error al detectar la rama: {e}")
    
    def push_personalizado(self):
        """Abre ventana para push personalizado"""
        ventana = tk.Toplevel(self.root)
        ventana.title("Push Personalizado")
        ventana.geometry("400x200")
        ventana.configure(bg='#2b2b2b')
        frame = ttk.Frame(ventana, padding="20")
        frame.pack(fill=tk.BOTH, expand=True)
        
        ttk.Label(frame, text="Remoto:").pack(anchor='w', pady=5)
        remoto_entry = ttk.Entry(frame, width=40)
        remoto_entry.insert(0, "origin")
        remoto_entry.pack(fill=tk.X, pady=5)
        
        ttk.Label(frame, text="Rama:").pack(anchor='w', pady=5)
        rama_entry = ttk.Entry(frame, width=40)
        rama_entry.insert(0, "main")
        rama_entry.pack(fill=tk.X, pady=5)
        
        def ejecutar_push():
            if remoto_entry.get() and rama_entry.get():
                self.ejecutar_comando(f'git push {remoto_entry.get()} {rama_entry.get()}')
                ventana.destroy()
            else:
                messagebox.showwarning("Advertencia", "Completa todos los campos")
        
        ttk.Button(frame, text="Push", command=ejecutar_push).pack(pady=10)
    
    def pull(self):
        """Pull automático desde origin con la rama actual"""
        timestamp = datetime.now().strftime('%H:%M:%S')
        self.escribir_salida(f"\n[{timestamp}] ", 'timestamp')
        self.escribir_salida("⬇️ Iniciando pull automático...\n", 'info')
        ...
        try:
            ...
            if rama_actual:
                self.escribir_salida(f"   Rama detectada: {rama_actual}\n", 'exito')
                self.escribir_salida(f"   Remoto: origin\n", 'info')
                self.ejecutar_comando(f'git pull origin {rama_actual}')
            else:
                self.escribir_salida("❌ No se pudo detectar la rama actual\n", 'error')
                messagebox.showerror("Error", "No se pudo detectar la rama actual.")
        except Exception as e:
